class MFreeSpinsWonPresentationView extends MView
{
	constructor()
	{
		super(0, 0, 1080, 1080);
		...
		this._fFreeSpinsWonChip_mfsacv = null;
	...
	init()
	{
		...
		//COIN...
		let l_mfsacv = new MFreeSpinsWonChipView();
		...
		this._fFreeSpinsWonChip_mfsacv = this.addChild(l_mfsacv);
		//...COIN

		//PARTICLES...
		let l_mbwppv = new MBigWinParticlesPoolView(this._fFreeSpinsWonChip_mfsacv);
		l_mbwppv.setXY(
			this.getWidth() / 2,
			this.getHeight() / 2);
		...
		l_mt.callFunctionOnStart(
			this._fFreeSpinsWonChip_mfsacv.displayValue,
			this._fFreeSpinsWonChip_mfsacv,
			0);
		...
		//COIN...
		l_mt.addAnimation(
			this._fFreeSpinsWonChip_mfsacv.setFlip,
