class MBackgroundCoinsPoolView extends MView
{
	static get COIN_GENERATION_INTERVAL_IN_FRAMES() { return 300 }

	constructor(aBubblePool_mbpv)
	{
		super(0, 0, 1920, 1080);

		this._fFramesCount_int = MBackgroundCoinsPoolView.COIN_GENERATION_INTERVAL_IN_FRAMES;
	...
	generateCoin(aX_num, aY_num)
	{
		...
		let l_mbcv = this.addChild(new MBackgroundCoinView(this._fBubblePool_mbpv));
		l_mbcv.reset(aX_num, aY_num);
	...
	onNextFrames(aFramesCount_int)
	{
		for( let i = 0; i < this._fCoins_mbcv_arr.length; i++ )
		{
			...
			this._fCoins_mbcv_arr[i].update(aFramesCount_int);
		}
		...
		if(this._fFramesCount_int > MBackgroundCoinsPoolView.COIN_GENERATION_INTERVAL_IN_FRAMES)
		{
			this._fFramesCount_int = Math.floor(this._fFramesCount_int % MBackgroundCoinsPoolView.COIN_GENERATION_INTERVAL_IN_FRAMES);
